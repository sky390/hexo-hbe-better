...
const knownPrefix = "<hbe-prefix></hbe-prefix>";
  ...
  async function verifyContent(key, content) {
    ...
    insertScript.text = '$("[class^=\'language-\']").attr("id", "code-block");\n'
    + '$("[class^=\'language-\']").addClass("ui segment");\n'
    + 'hljs.initHighlightingOnLoad();\n'
    + '$("table").addClass("ui table");';
    htm.appendChild(insertScript);
    if (!result) {
  ...
  async function decrypt(decryptKey, iv, hmacKey) {
    ...
    const result = await cryptoObj.subtle.decrypt({
      ...
      hideButton.classList.add("primary");
      hideButton.classList.add("basic");
